...
static irqreturn_t cpsw_interrupt(int irq, void *dev_id)
{
	...
	rx_thresh = __raw_readl(&priv->wr_regs->c0_rx_thresh_stat);
	rx = __raw_readl(&priv->wr_regs->c0_rx_stat);
	tx = __raw_readl(&priv->wr_regs->c0_tx_stat);
	if (!rx_thresh && !rx && !tx) {
				...
				struct ethtool_coalesce *coal)
{
	...
	writel(num_interrupts, &priv->wr_regs->c0_rx_imax);
	writel(num_interrupts, &priv->wr_regs->c0_tx_imax);
